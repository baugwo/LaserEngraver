...
from os import path
import time
starttime = time.strftime('%d-%m-%Y_%H_%M_%S', time.localtime(time.time()))
...
#User set variables
remdir = "/home/pi/laserengraver/"
address = "raspberrypi3.local" #This will be an IP address if avahi-daemon is not installed (which I strongly reccommend
user = "pi"
...
def engrave():
	...
	if proceed:
		...
		sshcmd("ssh "+user+"@"+address+" 'sudo -i nohup laserengraver -f "+remdir+filename+" -s "+str(speed)+" > "+remdir+filename+starttime+".txt' &")

def cancel():
	proceed = tkMessageBox.askokcancel(title="Are You Sure?", message="Engraving cannot be resumed after cancellation. Also Engraver needs to be recalibrated!")
	
	if proceed:
		sshcmd("ssh "+user+"@"+address+" 'sudo killall python'")
		
def shutdown():
	proceed = tkMessageBox.askokcancel(title="Are You Sure?", message="Engraving will stop, and cannot be resumed. After canceling check zero position engraver")
